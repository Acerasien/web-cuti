...
import { ExcuseReviewPanel } from "@/components/features/excuse/ExcuseReviewPanel";
...
export default async function ExcuseDetailPage({ params }: PageProps) {
  ...
  // Access guard: Karyawan can only view their own excuse requests
  if (!isAdmin && excuse.userId !== session.user.id) {
    redirect("/izin");
  }

  // Fetch reviewer details if reviewed
  let reviewerName = "";
  if (excuse.reviewedById) {
    const reviewer = await prisma.user.findUnique({
      where: { id: excuse.reviewedById },
      select: { name: true },
    });
  ...
  return (
    <PageWrapper title="Detail Pengajuan Izin">
      {/* Screen-Only Container */}
      <div className="screen-only-container">
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          {/* Header Row: Back Link & Action Buttons */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
            <Link href="/izin" className="btn btn-ghost btn-sm" style={{ display: "inline-flex", gap: 6 }}>
              <ArrowLeft size={16} /> Kembali ke Daftar
            </Link>
            <div style={{ display: "flex", gap: 8 }}>
              <PrintButton />
              <DownloadDocxButton type="excuse" data={JSON.parse(JSON.stringify(excuse))} reviewerName={reviewerName} />
            </div>
          </div>
            ...
            <div className="card-inner" style={{ padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "20px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
                <div style={{
                  ...
                  {excuse.user.name.split(" ").slice(0, 2).map((n) => n[0]).join("").toUpperCase()}
                </div>
                <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
                    <span className="badge badge-primary" style={{ fontSize: "10px", textTransform: "uppercase", padding: "2px 8px", fontWeight: 600 }}>
                      {excuse.user.role === "SUPERADMIN" ? "Super Admin" : excuse.user.role === "ADMIN" ? "Admin" : "Karyawan"}
                    </span>
              ...
              {/* Status block */}
              <div style={{
                background: excuse.status === "APPROVED" ? "var(--color-success-light)" : excuse.status === "REJECTED" ? "var(--color-danger-light)" : "var(--color-warning-light)",
                border: excuse.status === "APPROVED" ? "1px solid rgba(22, 163, 74, 0.15)" : excuse.status === "REJECTED" ? "1px solid rgba(220, 38, 38, 0.15)" : "1px solid rgba(217, 119, 6, 0.15)",
                ...
                <div style={{
                  ...
                  height: "36px",
                  borderRadius: "50%",
                  background: excuse.status === "APPROVED" ? "rgba(22, 163, 74, 0.1)" : excuse.status === "REJECTED" ? "rgba(220, 38, 38, 0.1)" : "rgba(217, 119, 6, 0.1)",
                  color: excuse.status === "APPROVED" ? "var(--color-success)" : excuse.status === "REJECTED" ? "var(--color-danger)" : "var(--color-warning)",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  flexShrink: 0
                }}>
                  {excuse.status === "APPROVED" && <CheckCircle size={18} />}
                  {excuse.status === "REJECTED" && <XCircle size={18} />}
                  {excuse.status === "PENDING" && <Clock size={18} />}
                </div>
                <div>
                  <span className={`badge badge-${excuse.status.toLowerCase()}`} style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", padding: "1px 6px", marginBottom: "4px" }}>
                    {excuse.status}
                  </span>
                  <p style={{ fontSize: "11px", color: "var(--color-text-muted)", margin: 0 }}>
                    {excuse.status === "APPROVED" && <>Disetujui pada <strong>{formatDate(excuse.reviewedAt || new Date())}</strong> oleh <strong>{reviewerName || "System"}</strong></>}
                    {excuse.status === "REJECTED" && <>Ditolak pada <strong>{formatDate(excuse.reviewedAt || new Date())}</strong> oleh <strong>{reviewerName || "System"}</strong></>}
                    {excuse.status === "PENDING" && <>Menunggu peninjauan oleh HRD / Atasan</>}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="card-outer" style={{ height: "100%" }}>
                <div className="card-inner" style={{ padding: "var(--space-5)", display: "flex", flexDirection: "column", gap: "20px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "14px", fontSize: "13px" }}>
                    <div>
                      <span className="text-muted" style={{ display: "block", fontSize: "10px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "2px" }}>Jenis Izin</span>
                    ...
                    <div>
                      <span className="text-muted" style={{ display: "block", fontSize: "10px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "2px" }}>Durasi</span>
                      <span style={{ fontWeight: 600, color: "var(--color-text)" }}>
                        {Number(excuse.totalDays || 0)} Hari
                      </span>
                    </div>

                    <div>
                      <span className="text-muted" style={{ display: "block", fontSize: "10px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "2px" }}>Tanggal Kejadian</span>
                      <span style={{ fontWeight: 600, color: "var(--color-text)" }}>
                        {isSingle ? formatDate(excuse.dateFrom) : `${formatDate(excuse.dateFrom)} - ${formatDate(excuse.dateTo)}`}
                      </span>
                    </div>
                    ...
                    <div>
                      ...
                      <span style={{ fontWeight: 500, color: "var(--color-text)" }}>
                        {new Date(excuse.createdAt).toLocaleDateString("id-ID", {
                          day: "numeric",
                          month: "long",
                          year: "numeric"
                        })} {new Date(excuse.createdAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}
                      </span>
                    </div>
                    ...
                    <div>
                      ...
                      <span style={{ fontWeight: 600, color: "var(--color-text)" }}>
                        {excuse.user.username ? `@${excuse.user.username}` : excuse.user.name}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="card-inner" style={{ padding: "var(--space-5)", display: "flex", flexDirection: "column", gap: "20px" }}>
                  ...
                  <div style={{ position: "relative", padding: "10px 0", flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                    <div style={{ position: "relative" }}>
                      ...
                      <div style={{ display: "flex", gap: "16px", position: "relative", zIndex: 1, marginBottom: "24px" }}>
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          <p style={{ fontSize: "11px", color: "var(--color-text-light)", margin: "2px 0" }}>
                            {new Date(excuse.createdAt).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })} {new Date(excuse.createdAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}
                          </p>
                          <p style={{ fontSize: "11px", color: "var(--color-text-muted)", margin: 0 }}>
                            oleh {excuse.user.name} ({excuse.user.username ? `@${excuse.user.username}` : "—"})
                          </p>
                        </div>
                      </div>
                      ...
                      <div style={{ display: "flex", gap: "16px", position: "relative", zIndex: 1, marginBottom: "24px" }}>
                        <div style={{
                          ...
                          height: "32px",
                          borderRadius: "50%",
                          background: excuse.status !== "PENDING" ? "var(--color-primary-light)" : "var(--color-warning-light)",
                          color: excuse.status !== "PENDING" ? "var(--color-primary)" : "var(--color-warning)",
                          display: "flex",
                          alignItems: "center",
                          justifyContent: "center",
                          flexShrink: 0
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          <p style={{ fontSize: "11px", color: "var(--color-text-light)", margin: "2px 0" }}>
                            {excuse.status !== "PENDING" && excuse.reviewedAt
                              ? `${new Date(excuse.reviewedAt).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })} ${new Date(excuse.reviewedAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}`
                              : "Sedang diproses"
                            }
                          </p>
                          <p style={{ fontSize: "11px", color: "var(--color-text-muted)", margin: 0 }}>
                      ...
                      <div style={{ display: "flex", gap: "16px", position: "relative", zIndex: 1 }}>
                        <div style={{
                          ...
                          height: "32px",
                          borderRadius: "50%",
                          background: excuse.status === "APPROVED" 
                            ? "var(--color-success-light)" 
                            : excuse.status === "REJECTED" 
                              ? "var(--color-danger-light)" 
                              : "var(--color-border)",
                          color: excuse.status === "APPROVED" 
                            ? "var(--color-success)" 
                            : excuse.status === "REJECTED" 
                              ? "var(--color-danger)" 
                          ...
                          {excuse.status === "APPROVED" && <CheckCircle size={16} />}
                          {excuse.status === "REJECTED" && <XCircle size={16} />}
                          {excuse.status === "PENDING" && <Clock size={16} />}
                        </div>
                        <div style={{ minWidth: 0 }}>
                          <h4 style={{ fontSize: "13px", fontWeight: 700, color: "var(--color-text)", margin: 0 }}>
                            {excuse.status === "APPROVED" ? "Disetujui" : excuse.status === "REJECTED" ? "Ditolak" : "Persetujuan"}
                          </h4>
                          <p style={{ fontSize: "11px", color: "var(--color-text-light)", margin: "2px 0" }}>
                            {excuse.status !== "PENDING" && excuse.reviewedAt
                              ? `${new Date(excuse.reviewedAt).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })} ${new Date(excuse.reviewedAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}`
                    ...
                    <div style={{
                      ...
                      {excuse.status === "APPROVED" && <CheckCircle size={14} />}
                      {excuse.status === "REJECTED" && <XCircle size={14} />}
                      {excuse.status === "PENDING" && <Clock size={14} />}
                      <span>
                        {excuse.status === "APPROVED" && "Permohonan izin ini telah disetujui."}
                        {excuse.status === "REJECTED" && "Permohonan izin ini telah ditolak."}
                        {excuse.status === "PENDING" && "Permohonan izin sedang dalam proses."}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="card-outer" style={{ height: "100%" }}>
                <div className="card-inner" style={{ padding: "var(--space-5)", display: "flex", flexDirection: "column", gap: "20px" }}>
                  ...
                  {excuse.attachmentUrl ? (() => {
                    const isImage = /\.(jpg|jpeg|png|webp|gif|svg)$/i.test(excuse.attachmentUrl);
                    const isPdf = /\.pdf$/i.test(excuse.attachmentUrl);
          ...
          {excuse.status === "PENDING" && isAdmin && (
            <div style={{ marginTop: "24px" }}>
              <ExcuseReviewPanel requestId={excuse.id} />
            </div>
          )}
        </div>
      </div>
      ...
      <div className="print-only-container">
        <div style={{ textAlign: "center", marginBottom: "24px" }}>
          <h2 style={{ fontSize: "20px", fontWeight: 700, textTransform: "uppercase", margin: 0 }}>Form Pengajuan Izin Karyawan</h2>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "24px" }}>
          ...
          <tbody>
            <tr>
              ...
              <td style={{ padding: "8px 0", fontSize: "14px", fontWeight: 600 }}>: {excuse.user.name}</td>
            </tr>
            <tr>
              <td style={{ padding: "8px 0", fontSize: "14px" }}>Jabatan / Posisi</td>
              <td style={{ padding: "8px 0", fontSize: "14px" }}>: {excuse.user.position || "—"}</td>
            </tr>
            <tr>
              <td style={{ padding: "8px 0", fontSize: "14px" }}>Departemen</td>
              <td style={{ padding: "8px 0", fontSize: "14px" }}>: {excuse.user.department || "—"}</td>
            </tr>
            <tr>
              <td style={{ padding: "8px 0", fontSize: "14px" }}>Email</td>
              <td style={{ padding: "8px 0", fontSize: "14px" }}>: {excuse.user.email || "—"}</td>
            </tr>
          </tbody>
        </table>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "24px" }}>
          ...
          <tbody>
            <tr>
              <td style={{ width: "30%", padding: "8px 0", fontSize: "14px" }}>Jenis Izin</td>
              <td style={{ padding: "8px 0", fontSize: "14px", fontWeight: 600 }}>: {getExcuseTypeLabel(excuse.excuseType)}</td>
            </tr>
            <tr>
              <td style={{ padding: "8px 0", fontSize: "14px" }}>Tanggal Kejadian</td>
              <td style={{ padding: "8px 0", fontSize: "14px" }}>
                : {isSingle ? formatDate(excuse.dateFrom) : `${formatDate(excuse.dateFrom)} s/d ${formatDate(excuse.dateTo)}`}
              </td>
            </tr>
            <tr>
              <td style={{ padding: "8px 0", fontSize: "14px" }}>Durasi Pengajuan</td>
              <td style={{ padding: "8px 0", fontSize: "14px" }}>: {Number(excuse.totalDays || 0)} Hari</td>
            </tr>
            <tr>
              <td style={{ padding: "8px 0", fontSize: "14px", verticalAlign: "top" }}>Alasan / Penjelasan</td>
              <td style={{ padding: "8px 0", fontSize: "14px", whiteSpace: "pre-wrap" }}>: {excuse.reason}</td>
            </tr>
          </tbody>
        </table>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "40px" }}>
          ...
          <tbody>
            <tr>
              ...
              <td style={{ padding: "8px 0", fontSize: "14px", fontWeight: 600 }}>: {excuse.status}</td>
            </tr>
              ...
              <>
                ...
                {excuse.reviewedAt && (
                  <tr>
                    ...
                    <td style={{ padding: "8px 0", fontSize: "14px" }}>: {formatDate(excuse.reviewedAt)}</td>
                  </tr>
                )}
                {excuse.rejectionNote && (
                  <tr>
                    <td style={{ padding: "8px 0", fontSize: "14px" }}>Catatan Penolakan</td>
                    <td style={{ padding: "8px 0", fontSize: "14px", color: "red" }}>: {excuse.rejectionNote}</td>
                  </tr>
                )}
              </>
            )}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: "60px" }}>
          <div style={{ textAlign: "center", width: "40%" }}>
            ...
            <p style={{ fontSize: "14px", fontWeight: 600, textDecoration: "underline", margin: 0 }}>{excuse.user.name}</p>
