...
import { reviewExcuseRequest } from "@/lib/actions/excuse";
import { Check, X, Loader2 } from "lucide-react";

interface ExcuseReviewPanelProps {
  requestId: string;
}

export function ExcuseReviewPanel({ requestId }: ExcuseReviewPanelProps) {
  ...
  const handleProcess = (status: "APPROVED" | "REJECTED") => {
    ...
    startTransition(async () => {
      const res = await reviewExcuseRequest(requestId, status, rejectionNote);
      if (res?.error) {
        setError(res.error);
      } else {
        router.refresh();
  ...
  return (
    <div className="card-outer mt-6" style={{ border: "1px solid rgba(249, 115, 22, 0.12)", background: "rgba(249, 115, 22, 0.01)" }}>
